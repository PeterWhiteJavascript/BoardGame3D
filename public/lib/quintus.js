...
var Quintus = exportTarget[key] = function(opts) {
    ...
    trigger: function(event, data) {
      ...
      if(this.listeners && this.listeners[event]) {
        ...
        for(var i=0,len = this.listeners[event].length;i<len;i++) {
          if(!this.listeners[event]) console.log(event,data,this.listeners[event][i])
          var listener = this.listeners[event][i];
          if(!listener) console.log(this.listeners,i,event,data)
  ...
  */
  Q.setup = function(id, options) {
    ...
    var w = parseInt(options.width || Q.el.width,10),
        h = parseInt(options.height || Q.el.height,10);
